 ...
 */
@Plugin(name = "DeeprouteThrowablePatternConverter", category = PatternConverter.CATEGORY)
@ConverterKeys({"deeprouteEx", "DeeprouteThrowable", "DeeprouteException"})
public class DeepRouteThrowablePatternConverter extends LogEventPatternConverter {
     ...
     * Options.
     */
    protected static ThrowableFormatOptions options;
     ...
     */
    @Deprecated
    protected DeepRouteThrowablePatternConverter(final String name, final String style, final String[] options) {
        this(name, style, options, null);
    }

    public static void updateLineNum(String maxLineNum) {
        DeepRouteThrowablePatternConverter.options = ThrowableFormatOptions.newInstance(new String[]{maxLineNum});
    }
     ...
     * @param config
     */
    protected DeepRouteThrowablePatternConverter(final String name, final String style, final String[] options, final Configuration config) {
        super(name, style);
        DeepRouteThrowablePatternConverter.options = ThrowableFormatOptions.newInstance(options);
        if (options != null && options.length > 0) {
            rawOption = options[0];
        }
        if (DeepRouteThrowablePatternConverter.options.getSuffix() != null) {
            final PatternParser parser = PatternLayout.createPatternParser(config);
            final List<PatternFormatter> parsedSuffixFormatters = parser.parse(DeepRouteThrowablePatternConverter.options.getSuffix());
        ...
        nonStandardLineSeparator = !Strings.LINE_SEPARATOR.equals(DeepRouteThrowablePatternConverter.options.getSeparator());
    }
     ...
     */
    public static DeepRouteThrowablePatternConverter newInstance(final Configuration config, final String[] options) {
        return new DeepRouteThrowablePatternConverter("Throwable", "throwable", options, config);
    }
